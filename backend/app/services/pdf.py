...
def document_pdf(kind: str, doc, company_profile: dict, template: dict | None = None) -> bytes:
    """Render a quotation or invoice to PDF.

    `doc` is a Quotation or Invoice ORM object; `template` is the customizable
    <kind>_template setting (title, accent_color, footer_note).
    """
    ...
    h1 = ParagraphStyle("h1", parent=styles["Title"], textColor=accent, alignment=0, fontSize=20)
    ...
    if footer_note:
        ...
        story.extend([Spacer(1, 10 * mm), Paragraph(str(footer_note), footer_style)])
